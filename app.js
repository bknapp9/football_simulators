...
let awayTeamGD = 0;
let done = false;
const GSMatchups = [[4, 2, 3, 1], [1, 4, 2, 3], [4, 3, 2, 1], [3, 2, 4, 1], [3, 4, 1, 2], [2, 4, 1, 3]]
let matchday = 1
...
const loadTeams = async () => {
    const res = await fetch('sud_teams.json'); // Fetch JSON data
...
loadTeams().then(Teams => {
    ...
    async function moveTeam(team, x, y) {
        ...
        if (x <= 4) {
            ...
            x = -2000 + ((x - 4) * 400);
            y = -280;
        } else if (qualifiedToQF.length === 0) { // Check if in group stage
            x = x % 2 === 1 ? -1200 : -700;
        } else {
            x = x % 2 === 1 ? -1300 : -650;
        }
    ...
    simulateMatchdayBtn.addEventListener('click', async (e) => {
        ...
        const homeTeamsIndex = [0, 6, 12, 18, 24, 30, 36, 42].map(x => x + clickCount - 1 % 6);
        ...
        for (let i = 0; i < homeTeams.length; i++) {
            ...
            const attendance = Math.round((attendancePercentage * capacity / 100)) + Math.floor(Math.random() * 1000) + 1;
    ...
    continueToKOBtn.addEventListener('click', (e) => {
        ...
        if (KORound === 0) {
            ...
            simulateKOBtn.addEventListener('click', async (e) => {
                ...
                const homeTeams = fixtureIdx.map(index => GSTableMatches[index].querySelectorAll('.homeTeamName')[clickCount-1].textContent);
                const awayTeams = fixtureIdx.map(index => GSTableMatches[index].querySelectorAll('.awayTeamName')[clickCount-1].textContent);
                ...
                for (let i = 0; i < homeTeams.length; i++) {
                    ...
                    firstLegHTScore = parseInt(GSTableMatches[fixtureIdx[i]].querySelector('.homeTeamScore').innerText) || 0;

                    let globalHTScore = firstLegHTScore + awayTeamGoals;

                    let firstLegATScore = parseInt(GSTableMatches[fixtureIdx[i]].querySelector('.awayTeamScore').innerText);
                    ...
                    console.log(firstLegHTScore, awayTeamGoals, firstLegATScore, homeTeamGoals, globalHTScore, globalATScore)
                }
                    ...
                    let currentRow = GSTableMatches[fixtureIdx[i]].querySelectorAll('.fixtureRow:not(.done)')[0]
                    const detailsElementDiv = currentRow.parentNode.querySelector('.details');
                    if (!detailsElementDiv) {
                        currentRow.parentNode.insertBefore(detailsElement, currentRow.nextSibling);
                    } 
                    ...
                    if (clickCount === 2) {
                        ...
                        GSTableMatches[fixtureIdx[i]].querySelector('.globalScore').classList.remove('hide');
                        globalScoreValue = GSTableMatches[fixtureIdx[i]].querySelector('.globalScoreValue');
        ...
        function populateKnockoutStage(teamNamesElements, teamImgsElements, qualifiedTeams, increment) {
            let potIndex = 0;
            let fixtureIdx = 0;
            ...
            for (let i = 0; i < qualifiedTeams.length; i++) {
                ...
                let nameIndex1 = fixtureIdx % 2 === 0 ? potIndex + fixtureIdx + (fixtureIdx + 1) : potIndex + fixtureIdx + (fixtureIdx - 2);
                ...
                teamImgsElements[imgIndex1].src = `images/${normalizedTeam.replace(/\s+/g, '')}.png`;
                teamImgsElements[imgIndex2].src = `images/${normalizedTeam.replace(/\s+/g, '')}.png`;
